...
def get_group_annotation(df, his_len=288, pred_len=24, time_interval="5min"):
    """
    根据历史长度和预测长度对数据进行分组，并按照工作日和周末模式进行分类。
    ...
    :return: 两种模式的连续分组列表 (groups_mode_0_res, groups_mode_1_res)
    """
    # 获取连续性分组，初步按照时间间隔划分
    ...
    for group in all_groups:
        ...
        # 添加一个新的列 "mode"，根据日期判断工作日 (mode=1) 或周末 (mode=0)
        group["mode"] = group["kpi_time"].dt.weekday.apply(lambda x: 1 if x < 5 else 0)
        
        # 遍历当前分组，按照历史长度和预测长度拆分样本
        for i in range(0, len(group) - his_len - pred_len):
            # 提取当前样本，包含历史数据和预测数据
            group_item = group.iloc[i: i + his_len + pred_len]
            
            # 统计预测数据段中 (倒数 pred_len 个元素) 的 mode 为 0 的个数
            num_mode_0 = np.sum(group_item["mode"][-pred_len:] == 0)
            num_mode_1 = pred_len - num_mode_0  # mode 为 1 的个数
            
            # 根据 mode 的数量将样本分配到对应的模式分组
            if num_mode_0 > num_mode_1:
                groups_mode_0.append(group_item)
            else:
                groups_mode_1.append(group_item)
    
    # 将每个模式的样本合并为一个 DataFrame，并去重
    groups_mode_0 = pd.concat(groups_mode_0, ignore_index=True).drop_duplicates(subset=["kpi_time"])
    groups_mode_1 = pd.concat(groups_mode_1, ignore_index=True).drop_duplicates(subset=["kpi_time"])

    # 对每个模式的分组再次按照时间间隔进行连续性划分
    groups_mode_0_res = get_continuous_group(groups_mode_0, time_interval)
    groups_mode_1_res = get_continuous_group(groups_mode_1, time_interval)
    
    # 返回两种模式的分组结果
    return groups_mode_0_res, groups_mode_1_res
