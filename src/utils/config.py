...
def get_args():
    ...
    parser.add_argument(
        "--data_multi_basedir", type=str, default=None, help="数据的基础文件夹"
    )
    parser.add_argument(
        "--data_path",
        type=str,
    ...
    parser.add_argument("--his_hour", type=int, default=24, help="input sequence hour")
    parser.add_argument("--pred_hour", type=int, default=6, help="start token hour")
    ...
    if 'gunicorn' in sys.argv[0] or "ipykernel_launcher" in sys.argv[0]:
        # When running under gunicorn, use default values
        args = parser.parse_args([])
    else:
        # When running directly, parse command line arguments
        args = parser.parse_args()
    return args
